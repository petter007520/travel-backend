    ...
    public function settop(Request $request)
    {
        if($request->isMethod("post")){
          $identityInfo =  DB::table($this->table)
             // ->select('status','userid','realname','idnumber')
            ...
            if($data['status'] == 1 && !$check){
                ...
                $Memberzc = Member::find($member->top_uid);
                if(!empty($Memberzc)){
                    $Memberzc->increment('zc_num',1);
                    $Memberzc->increment('tree_zc',1);

                    //邀请注册赠送能量金
                    $yqnl = DB::table('setings')->where(['keyname'=>'nl_yq'])->value('value');//注册赠送金额
                    if((int)$yqnl > 0){
                        $yuannl1 = $Memberzc->nl_fee;
                        $Memberzc->increment("nl_fee",(int)$yqnl);
                        $notice = "下级注册获取希望资金";
                        $bigtree = Bigtree::find(1);
                        $bigtree->increment("nl",(int)$yqnl);
                        $log=[
                            "userid"=>$Memberzc->id,
                            "username"=>$Memberzc->username,
                            "money"=>$yqnl,
                            "notice"=>$notice,
                            "type"=>"下级注册获取希望资金",
                            "status"=>"+",
                            "yuanamount"=>$yuannl1,
                            "houamount"=>$Memberzc->nl_fee,
                ...
                $zctreeid = DB::table('setings')->where('keyname','zc_treeid')->value('value');  //注册赠送小树
                $tree_ids = explode(',',$zctreeid);
                if(!empty($tree_ids)){
                    foreach ($tree_ids as $value) {
                        $treeinfo = TreeProduct::find($value);
                ...
                $zcnl = DB::table('setings')->where(['keyname'=>'nl_zc'])->value('value');//注册赠送金额
                if((int)$zcnl > 0){
                    $yuannl = $member->nl_fee;
                    $member->increment("nl_fee",(int)$zcnl);
                    $notice = "注册获取希望资金";
                    $bigtree = Bigtree::find(1);
                    $bigtree->increment("nl",(int)$zcnl);
                    $log=[
                        "userid"=>$member->id,
                        "username"=>$member->username,
                        "money"=>$zcnl,
