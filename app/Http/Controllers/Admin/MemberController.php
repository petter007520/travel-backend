...
class MemberController extends BaseController
{
    ...
    public function __construct(Request $request)
    {
        ...
        $membergrouplevel= DB::table("membergrouplevel")->orderBy("id","asc")->get();
        $membergrouplevelName=[];
        foreach($membergrouplevel as $item){
            $membergrouplevelName[$item->id]=$item->name;
        }
        $this->membergrouplevelName=$membergrouplevelName;
        ...
        $dividend_types = DB::table('dividend_type')->get();
        $this->dividend_type =$dividend_types;

        view()->share("dividend_type",$this->dividend_type);
    }
        ...
        if($request->ajax()){
            ...
            $list = DB::table($this->table)
                ...
                ->where(function ($query) {
                    ...
                    if(isset($_REQUEST['s_is_ysh']) && $_REQUEST['s_is_ysh']!=''){
                        $query->where("is_ysh","=",$_REQUEST['s_is_ysh']);
                    }
                })
        ...
        if($request->isMethod("post")){
            ...
            if($request->input("username")){
                ...
                if($admin){
                    ...
                    if($request->ajax()){
                        return response()->json(["msg"=>"用户名已经存在","status"=>1]);
                    }else{
            ...
            if($request->input("mobile")){
                ...
                $phoned = \App\Member::EncryptPassWord($request->input("mobile"));

                $PHONEex= DB::table($this->table)->where(['mobile' => $phoned])->first();
                if($PHONEex){
                    if($request->ajax()){
            ...
            DB::table($this->table)->where("id",$id)->update(["invicode"=>($id+312866)]);
            DB::table('statistics')->insert(["user_id"=>$id,"username"=>decrypt($data['mobile']),'register_date'=>date('Y-m-d'),'created_at'=>Carbon::now()]);
           //后台统计
            ...
            if($data['inviter']){
                \App\Member::Upgrade($data['inviter']);
            }

                $upinfo = DB::table($this->table)->select('id','top_uid')->where('invicode',$data['inviter'])->first();


            if($upinfo){
                $upinfo1 = \App\Member::find($upinfo->id);
                //$upinfo1->increment('zc_num',1);
               // $zc_num = DB::table("setings")->where('keyname','zc_num')->value('value');
                /*if($upinfo1->rw_leve == 0 &&$upinfo1->zc_num >= $zc_num){
                    $upinfo1->increment('rw_level',1);
                }*/
                DB::table($this->table)->where("id",$id)->update(["top_uid"=>$upinfo->id,'ttop_uid'=>$upinfo->top_uid]);
            }
            /*
            $Member = Member::where();
...
*/
        ...
        DB::beginTransaction();
        try{
            $member_info = DB::table($this->table)->select('id','username','amount','integral','activation','top_uid','mtype','qd_count')->where("id",$id)->first();

            // if($member_info->mtype != $data['mtype']){
            //     if($member_info->qd_count > 0 ){
            //         DB::rollBack();
            //         return ['status'=>0,'msg'=>'请勿多次修改该账户分红方式'];
            //     }
            //     if($member_info->mtype == 0){
            //         DB::rollBack();
            //         return ['status'=>0,'msg'=>'请先让用户自己选择分红方式'];
            //     }
            //     $data['qd_count'] = 1;
            // }
    ...
    public function set_ysh(Request $request){
    ...
    public function luckdraws(Request $request){
        ...
        $Member=Member::find($request->id);
        if($Member && $request->luckdraws>0){

            $Member->increment('luckdraws', $request->luckdraws);
            $msg = [
                "userid" => $Member->id,
                "username" => $Member->username,
                "title" => "赠送抽奖券",
                "content" => "赠送您(" . $request->luckdraws . ")抽奖券",
                "from_name" => "系统通知",
                "types" => "抽奖",
            ];
    ...
    public function moneys(Request $request){
        ...
        if($request->isMethod("post")){
            ...
            if($request->moneytype=='+') {
                ...
                $notice = $request->memo != '' ? $request->memo:'';
                $log = [
                    "userid" => $Member->id,
                    "username" => $Member->username,
...
// $parm = $request->toArray();
    ...
    //签到日历
    public function sign_log(Request $request){
        $user_id = $request->userid;
        $year = date('Y');
        $month = date('m');
        $signlog = DB::table('signlog')->where(['sign_year'=>$year,'sign_month'=>$month])->first();
        if(!$signlog){
             return response()->json(["msg" => "查无数据", "status" => 1]);
        }

        $multiplied = $collection->map(function ($item, $key) {
            return $item * 2;
        });
        return response()->json(["msg"=>"修改成功","status"=>0,'data'=>$data]);
    }
