...
class ProductqishuController extends BaseController
{

    private $table="productqishu";
    ...
    public function lists(Request $request){
        ...
        $listDB = DB::table($this->table)
            ->leftJoin('products', 'products.id', '=', $this->table.'.productid')
    ...
    public function iskq(Request $request)
    {
        if($request->isMethod("post")){

            /*$data = DB::table($this->table)->find($request->input('id'));

            $data['is_kq'] = $request->input('is_kq');*/

            DB::table($this->table)->where("id",$request->input('id'))->update(['is_kq'=>$request->input('is_kq')]);
