...
class SalesmansController extends BaseController
{
        ...
        if($request->ajax()){
            ...
            if($list){
                foreach ($list as $item){
                    ...
                    $item->xxtuiguangrens=count($item->tuiguangren);
                    $item->xxwithdrawals=DB::table("memberwithdrawal")->where("status","1")->whereIn("userid",$item->tuiguangren)->sum("amount");
                    $item->xxrecharges=DB::table("memberrecharge")->where("status","1")->whereIn("userid",$item->tuiguangren)->where("type","<>","优惠活动")->sum("amount");
                    $item->xxbuys=DB::table("productbuy")->whereIn("userid",$item->tuiguangren)->sum("amount");
                    $item->xxmoneys=$item->recharges-$item->withdrawals;
                }
