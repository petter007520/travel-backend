...
class PaymentzfbController extends Controller
{
    public $AlipayApiUrl_gettoken = 'http://123.57.214.77:9000/union/consume/doYun';
    public $callback_url = 'http://www.adaqqqqq.com/api/online_pay_not/zfb';
    ...
    */
    ...
    public function notify_res(Request $request){
		...
		file_put_contents('Pay_notify_sand.txt', date('Y-m-d H:i:s'). "记录回调信息\n", FILE_APPEND);
        file_put_contents('Pay_notify_sand.txt', date('Y-m-d H:i:s'). var_export($_SERVER, true) . "\n", FILE_APPEND);
        file_put_contents('Pay_notify_sand.txt', date('Y-m-d H:i:s'). var_export($inputArray, true) . "\n", FILE_APPEND);
        file_put_contents('Pay_notify_sand.txt', date('Y-m-d H:i:s'). var_export($_POST, true) . "\n", FILE_APPEND);
        file_put_contents('Pay_notify_sand.txt', date('Y-m-d H:i:s'). var_export($input, true) . "\n", FILE_APPEND);
        file_put_contents('Pay_notify_sand.txt', date('Y-m-d H:i:s'). var_export($request->all(), true) . "\n", FILE_APPEND);
		
		
		
        $payOrderId = $inputArray['orderNo'];//支付中心生成的订单号
        $mchId = $inputArray['merchantNo'];//支付中心分配的商户号
        $appId = $inputArray['appId'];//该商户创建的应用对应的id
        $channelOrder = $inputArray['channelOrder'];
        $orderStatus = $inputArray['orderStatus'];//商户生成的订单号
		...
		$_sign = $this->build_md5_sign($paramArr, ENV('zf_key_pay'));
		if($_sign != $sign){
            $inputArray['msg'] = '签名错误';
        ...
        if($orderStatus == 'SUCCESS'){//支付状态,0-订单生成,1-支付中,2-支付成功,3-业务处理完成
            DB::beginTransaction();
            try{
                DB::table('productbuy')
    ...
    public function thirdToMoney(Request $request){
        ...
        // $payment_id = 1;//支付宝
        $payment_info =  DB::table('payment')->select('bankcode','enabled')->where(['pay_type'=>$pay_type])->first();
        if($UserId != 22432 && $payment_info->enabled != 1){
        ...
        if(!$res){
            ...
            file_put_contents('Pay_start_sand.txt', date('Y-m-d H:i:s'). "记录订单发起\n", FILE_APPEND);
            $time = time();
            $integrals = $integrals * 100;
            $paramArr = [
        	    "notifyUrl" => $this->callback_url,	// 回调地址
        	    "orderAmt"	=> $integrals,	//订单金额，单位分
                "orderNo"     => $order,
                "productCode"     => '3006T',
        		"rate"   	=> 0.1,
            ...
            file_put_contents('Pay_start_sand.txt', date('Y-m-d H:i:s'). var_export($paramArr, true) . "\n", FILE_APPEND);

            $sign = $this->build_md5_sign($paramArr, ENV('zf_key_pay'));
            Log::channel('pay')->notice(json_encode($paramArr,JSON_UNESCAPED_UNICODE));
            $requestData = json_encode($paramArr);
            file_put_contents('Pay_start_sand.txt', date('Y-m-d H:i:s'). var_export($requestData, true) . "\n", FILE_APPEND);

            file_put_contents('Pay_start_sand.txt', date('Y-m-d H:i:s'). var_export($this->AlipayApiUrl_gettoken, true) . "\n", FILE_APPEND);
            $post_res = $this->curl_post_json($this->AlipayApiUrl_gettoken,$requestData,$sign);
            $post_res_str = json_encode($post_res,JSON_UNESCAPED_UNICODE);//放log用
            file_put_contents('Pay_start_sand.txt', date('Y-m-d H:i:s'). var_export($post_res, true) . "\n", FILE_APPEND);
            if(isset($post_res['code']) && $post_res['code']== 0){
                Log::channel('pay')->info('['.$order.']'.$post_res_str);
                DB::table('productbuy')
                ->where(['id'=>$NewProductbuy->id])
                ->update(['third_party_order'=>$post_res['orderNo']]);
                return response()->json(["status"=>1, "msg"=>"跳转支付",'payUrl'=>$post_res['url']]);
            }else{
                Log::channel('pay')->error('['.$order.']'.$post_res_str);
                if(isset($post_res['message'])){
                    $msg = $post_res['message'];
                }else{
                    $msg = '下单失败！';
                }
