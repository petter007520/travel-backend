...
class UserController extends Controller
{
    ...
    public function teamReport(Request $request){
        ...
        $my_statistics = DB::table("statistics")
            ...
            // ->orwhere('top_three_uid',$UserId)
            // ->orwhere('top_four_uid',$UserId)
            // ->orwhere('top_five_uid',$UserId)
            ->get();
        ...
        //团队总提现
        // $team_total_withdrawal = DB::table('statistics')
        //     ->select('team_capital_flow','top_one_uid','top_two_uid','top_three_uid','top_four_uid','top_five_uid')
        //     ->where('top_one_uid',$UserId)
        //     ->orwhere('top_two_uid',$UserId)
            // ->orwhere('top_three_uid',$UserId)
            // ->orwhere('top_four_uid',$UserId)
            // ->orwhere('top_five_uid',$UserId)
            // ->get();
            // ->sum('team_total_withdrawal');
        $team_total_withdrawal = $my_statistics->sum('team_total_withdrawal');
        //团队认购总额
        // $level_one_capital = $level_two_capital = $level_three_capital = $level_four_capital = $level_five_capital = 0;
        // foreach ($my_statistics as $v){
        //     if($v->top_one_uid == $UserId){
        //         $level_one_capital += $v->capital_flow;
        //     }
        //     if($v->top_two_uid == $UserId){
        //         $level_two_capital += $v->capital_flow;
        //     }
            // if($v->top_three_uid == $UserId){
            //     $level_three_capital += $v->capital_flow;
            // }
            // if($v->top_four_uid == $UserId){
            //     $level_four_capital += $v->capital_flow;
            // }
            // if($v->top_five_uid == $UserId){
            //     $level_five_capital += $v->capital_flow;
            // }
        // }
        ...
        switch ($level_type) {
            ...
            case 2:
                ...
                break;
            // case 3:
            //     $level_id_arr = $level_three;
            //     break;
            // case 4:
            //     $level_id_arr = $level_four;
            //     break;
            // case 5:
            //     $level_id_arr = $level_five;
            //     break;
            }
        $level_info = Member::select('id','nickname','username','picImg','created_at')->where($where)->whereIn('id',$level_id_arr)->paginate($pageSize);
        // $level_statistics = DB::table('statistics')->whereIn('user_id',$level_id_arr)->get();
        // $level_statistics_arr = [];
        // foreach ($level_statistics as $level_sta){
        //     $level_statistics_arr[$level_sta->user_id] = $level_sta;
        // }
        foreach ($level_info as $v){
            ...
            // $v->capital_flow = $level_statistics_arr[$v->id]->capital_flow;
            // $v->team_total_withdrawal = $level_statistics_arr[$v->id]->team_total_withdrawal;
        }

        //首充人数
        // $first_charge_count = $first_charge_count;
        //直推人数
        ...
        // $data['total_reward_amount'] = sprintf("%.2f",$total_reward_amount);//总推荐奖励
        ...
        // $data['level_one_capital'] = sprintf("%.2f",$level_one_capital);
        // $data['level_two_capital'] = sprintf("%.2f",$level_two_capital);
        // $data['level_three_capital'] = sprintf("%.2f",$level_three_capital);
        // $data['level_four_capital'] = sprintf("%.2f",$level_four_capital);
        // $data['level_five_capital'] = sprintf("%.2f",$level_five_capital);

        // $data['level_one_count'] = count($level_one);
        // $data['level_two_count'] = count($level_two);
        // $data['level_three_count'] = count($level_three);
        // $data['level_four_count'] = count($level_four);
        // $data['level_five_count'] = count($level_five);
    ...
    public function nowToMoney(Request $request){
        ...
        $product=DB::table("products")
           // ->select('id','title','category_id','qtje','isft','tzzt','hkfs','shijian','zgje','qxdw','zsje','zsje_type','jyrsy','qtsl','zscp_id','fy_type')
            ...
            ->first();
        if($product->category_id == 42 && $request->amount <= 0){
            return response()->json(["status"=>0,"msg"=>"购买金额错误！"]);
        }
        //福利产品限制
        if($product->category_id ==44){
            /////限购数量
            $flxg = DB::table('setings')->where(['keyname'=>'fl_xg'])->value('value');
            $llnumg =   DB::table("productbuy")
                ->where("userid",$UserId)
        ...
        if($Member->rw_level<1 && $product->category_id ==13){
           // return response()->json(["status"=>0,"msg"=>"请先完成注册任务！"]);
        }
       /* if($Member->rw_level<3 && $product->category_id ==42){
            return response()->json(["status"=>0,"msg"=>"福利产品购买！"]);
        }*/
        $integrals= $product->qtje*$request->number;
        //判断是否为余额宝
        if($product->category_id==42){
            $integrals = abs($request->amount);
        }
        // if($product->category_id == 11){
        //     $integrals = (($product->qtje * 0.01 * $product->jyrsy) + $product->qtje)*$request->number;
        // }
        ...
        //     return response()->json(["status"=>0,"msg"=>"消费积分不足,当前积分余额：".$Member->integral]);
        // }
        //下次第一期分红时间
        ...
        /*if($product->category_id==42){
            //余额宝下次退款时间
            $useritem_time2 = \App\Productbuy::DateAdd("d",$product->shijian, date('Y-m-d 0:0:0',time()));
        }*/
		...
		//try{
			// if($integral > 0){
			//     $real_amount = $integrals - $integral;
			//     $Member->decrement('integral',$integral);
			...
			if($pay_type == 1){
				// $de_integrals = $integral>0?$real_amount:$integrals;
                if($fhtype==1){
                    $yuanamount = $Member->ktx_amount;
			...
			if(!$res){
				...
				if($pay_type == 1){//如果是余额支付
					...
					//添加个人统计
					if($product->category_id == 12){
						DB::table('statistics')->where('user_id',$Member->id)->update([
							'capital_flow' => DB::raw("capital_flow +". $capital_flow),
							'equity_capital_flow' => DB::raw("equity_capital_flow + " . $capital_flow)
							]);
						//是否满足额外分红
						$equity_capital_flow = DB::table('statistics')->where('user_id',$Member->id)->value('equity_capital_flow');
						if ($equity_capital_flow != null) {
							...
							$user_extra_bonus = DB::table('extra_bonus')->where('uid',$Member->id)->first();
						} else {
						...
						if($extra_bonus_type){
							...
							if($user_extra_bonus){
								...
								if($user_extra_bonus->type_id < $extra_bonus_type->id){
									...
									DB::table('extra_bonus')->where('uid',$Member->id)->update($update_extra_bonus);
								}
							}else{
								$inser_extra_bonus = [
									'uid'=>$Member->id,
