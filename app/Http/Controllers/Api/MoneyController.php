...
class MoneyController extends Controller
{
    ...
    public function recharge(Request $request){
        ...
        $payment_certificate = $request->post('payment_certificate','');
        ...
        if($payment->type == 'chain'){
            $type = 2;
        }
        $memo= $payment->pay_name.'充值'.$amount;
        //USDT支付
        $usdt_amount = 0;
        if($payment->pay_type == 5){
         ...
         Memberrecharge::Recharge([
            ...
            "recharge_type"=>$type,
            "payimg"=>$payment_certificate,//支付凭证
        ]);
    ...
    public function withdraw(Request $request)
    {
        ...
        if($type==3){
            $withdrawalmin = Cache::get("withdrawMinUsdt");
            if(!$withdrawalmin){
                $withdrawalmin = Db::table("setings")->where('keyname','withdrawMinUsdt')->value('value');
            }
            if($amount<$withdrawalmin){
                return response()->json(["status"=>0, "msg"=>"最低提款数量为".$withdrawalmin]);
            }
        }
        ...
        if($type==1 && $Member->ktx_amount<$amount){
            return response()->json(["status"=>0, "msg"=>"帐户余额不足"]);
        }
        if($type==3 && $Member->usdt_amount<$amount){
            return response()->json(["status"=>0, "msg"=>"帐户余额不足"]);
        }
