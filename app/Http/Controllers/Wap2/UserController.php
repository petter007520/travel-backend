...
class UserController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $data['mobile'] = \App\Member::half_replace(\App\Member::DecryptPassWord($this->Member->mobile));  //手机号
        $data['levelname'] = $this->memberlevelName[$this->Member->level];   //vip名称

        $data['all_amount'] = sprintf("%.2f",$this->Member->amount+$this->Member->is_dongjie);//我的资产
        $data['amount'] = sprintf("%.2f",$this->Member->amount);   //可用余额
        $data['moneylog_moneys']=  DB::table("moneylog")->where("moneylog_userid",$UserId)->where("moneylog_type","项目分红")->sum("moneylog_money"); //累计收益
        ...
        if($request->ajax()){
           ...
           if($EditMember){
               ...
               if($request->mobile!=''){
                  ...
                  if(strlen($mobile) != 11){
                      return response()->json(['status'=>1,'msg'=>"您输入的手机位数不对"]);
                  }
                  $EditMember->mobile = \App\Member::EncryptPassWord($mobile);
               }
               ...
               if($request->old_pwd!='' && $request->new_pwd!=''){ 
                  ...
                  if(strlen(trim($request->new_pwd)) < 6){
                      return response()->json(['status'=>1,'msg'=>"新密码不得少于6位"]);
                  }
               ...
               if($request->old_paypwd!='' && $request->new_paypwd!=''){ 
                  ...
                  $paywpd = \App\Member::DecryptPassWord($EditMember->paywpd);

                  if($paywpd != $request->old_paywpd){
                     return response()->json(["status"=>1,"msg"=>"原支付密码错误"]);
                  }

                  if(strlen(trim($request->new_paywpd)) < 6){
                      return response()->json(['status'=>1,'msg'=>"新支付密码不得少于6位"]);
                  }

                  $EditMember->paywpd = \App\Member::EncryptPassWord(trim($request->new_paywpd));
        ...
        if($request->ajax()){
           ...
           if($EditMember){
               ...
               if($EditMember->bankname==''){
                   ...
                   return response()->json(["status"=>1,"msg"=>"银行卡名称不能为空"]);
               }
               ...
               if($EditMember->bankrealname==''){
                   ...
                   return response()->json(["status"=>1,"msg"=>"开户人不能为空"]);
               }
               ...
               if($EditMember->bankcode==''){
                   ...
                   return response()->json(["status"=>1,"msg"=>"银行卡号不能为空"]);
               }
               ...
               if($EditMember->bankaddress==''){
                   ...
                   return response()->json(["status"=>1,"msg"=>"开户行不能为空"]);
               }
