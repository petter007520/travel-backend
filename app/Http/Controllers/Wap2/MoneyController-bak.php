    ...
    public function payconfig(Request $request){
              ...
              if($Payment){
                  ...
                  $html=' <div class="formGroup"><span class="left">帐户可用余额</span> <span class="right">¥<font style="color:#F00; font-size:22px;font-weight:700">'.sprintf("%.2f",$this->Member->amount).' </font>元</span></div>
                   <div class="formGroup"><span class="left">充值金额</span> <input class="right" type="number" name="amount" id="price" value="" maxlength="8" placeholder="输入充值金额"></div>
                  <div class="formGroup"><span class="left">充值方式</span>
                   <div id="parent">
                <select onchange="payconfig(this.value)" class="right finance_type">
                    <option value="">请选择充值方式</option>';
                        ...
                        foreach($this->payment as $pk=>$Pay){
                            ...
                            $html.=' <option value="'.$pk.'" '.$selected.' >'.$Pay.'</option>';
                        }
