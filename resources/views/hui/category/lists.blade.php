...
@section('form')
    ...
    <script id="demo" type="text/html">
        ...
        <tr>
            ...
            <td><% item.thumb_url?'<img src="'+item.thumb_url+'" width="20" onmouseover="this.width=100" onmouseout="this.width=20"/>':'' %></td>
        ...
        function pagelist_tree(list,i){
            ...
            layui.use(['laytpl','form'], function(){
                ...
                layui.each(list, function(index, item){
                    ...
                    if(item.thumb_url){
                        _html+='  <td><img src="'+item.thumb_url+'" width="20" onmouseover="this.width=100" onmouseout="this.width=20"/></td>';
                    }else{
                        _html+='  <td></td>';
                    }
                    ...
                    if(item.atindex==1){
                        _html+='  <input type="checkbox"  checked lay-skin="switch" lay-filter="switchTest-atindex" lay-text="首页|隐藏" id="'+item.id +'">';
                       }else{
                        _html += '  <input type="checkbox"      lay-skin="switch" lay-filter="switchTest-atindex" lay-text="首页|隐藏" id="'+item.id +'">';
                    ...
                    if(item.atfoot==1){
                        _html+='  <input type="checkbox"  checked lay-skin="switch" lay-filter="switchTest-atfoot" lay-text="底部|隐藏" id="'+item.id +'">';
                    }else{
                        _html += '  <input type="checkbox"      lay-skin="switch" lay-filter="switchTest-atfoot" lay-text="底部|隐藏" id="'+item.id +'">';
                    ...
                    if(item.ismenus==1){
                        _html+='  <input type="checkbox"  checked lay-skin="switch" lay-filter="switchTest-ismenus" lay-text="导航|隐藏" id="'+item.id +'">';
                       }else{
                        _html += '  <input type="checkbox"      lay-skin="switch" lay-filter="switchTest-ismenus" lay-text="导航|隐藏" id="'+item.id +'">';
                    ...
                    _html+='  <a title="删除" onclick="del('+ item.id +',1)" href="javascript:;">';
                    _html+='      <i class="layui-icon">&#xe640;</i>';
                    _html+='  </a>';
                    _html+='  </td>';
                    _html+='  </tr>';
       ...
       layui.use(['form'], function(){
        ...
        var form = layui.form;
           ...
           form.on('switch(switchTest-atindex)', function(data){
               ...
               var load;
               $.ajax({
                   url: "{{ route($RouteController.'.atindex') }}",
                   type:"post",     //请求类型
                   data:{
                       id:id,atindex:atindex,
