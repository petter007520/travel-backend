        ...
        <div class="layui-row">
            <form class="layui-form layui-col-md12 x-so" action="{{ route($RouteController.".lists") }}" method="get">
                ...
                <div class="layui-input-inline">
                    ...
                    <select name="s_celebritytype" lay-filter="celebritytype">

                        <option value="">名家品牌</option>
                        @foreach( explode("|",\Cache::get('CelebrityType')) as $k=>$itme)
                                <option value="{{$itme}}" @if(isset($_REQUEST['s_celebritytype']) && $_REQUEST['s_celebritytype']==$itme) selected="selected" @endif>{{$itme}}</option>
                        @endforeach
    ...
    <script id="demo" type="text/html">
            ...
            <tr>
                ...
                <td>
                    ...
                    <input type="checkbox"   lay-skin="switch" lay-filter="switchTest-settop" lay-text="置顶|普通" id="<% item.id %>">
                    <%# }else{ %>
                    <input type="checkbox"    checked  lay-skin="switch" lay-filter="switchTest-settop" lay-text="置顶|普通" id="<% item.id %>">
        ...
        form.on('select(category_id)', function(data){
           ...
           // console.log(data.othis); //得到美化后的DOM对象
            var obj={
                ...
                s_celebritytype:$("[name='s_celebritytype']").val(),
            };
            lists(1,obj);
        });

        form.on('select(celebritytype)', function(data){
            //console.log(data.elem); //得到select原始DOM对象
            console.log(data.value); //得到被选中的值
            //console.log(data.othis); //得到美化后的DOM对象
