...
@section('form')
    ...
    <script id="demo" type="text/html">
        ...
        <tr>
            ...
            <td><% item.value %></video></td>
            <td><% item.thumb_url?'<img src="'+item.thumb_url+'" width="20"/>':'' %></td>

            <td><% item.sort %></td>
            <td>
                <%# if(item.status==0){ %>
                <input type="checkbox"   lay-skin="switch" lay-filter="switchTest-setstatus" lay-text="显示|隐藏" id="<% item.id %>">
                <%# }else{ %>
                <input type="checkbox"    checked  lay-skin="switch" lay-filter="switchTest-setstatus" lay-text="显示|隐藏" id="<% item.id %>">
                <%# } %>
            </td>
